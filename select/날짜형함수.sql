...
select date_format(now(),'%Y년 %m월 %d일 %h시 %i분 %s초');
select now(), date_add(now(), INTERVAL 6 day) '6day', date_add(now(), INTERVAL 6 month) '6month', date_add(now(), INTERVAL 6 year);
select period_diff(date_format(now(),'%Y%m%d'), date_format(date_add(now(),INTERVAL 6 month),'%Y%m%d'));