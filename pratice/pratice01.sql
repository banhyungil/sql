...
-- 출력은 이름, 성별,  입사일 순서이고 “이름”, “성별”, “입사일로 컬럼 이름을 대체해 보세요.
   select concat(first_name, ' ', last_name) 이름, gender 이름, hire_date 입사일
   from employees
   order by hire_date;
-- 문제3
-- 여직원과 남직원은 각 각 몇 명이나 있나요?
	select case gender when 'M' then '남직원' when 'F' then '여직원' end 성별, count(*)
    from employees
    group by gender;
-- 문제4
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) 
	select count(distinct(emp_no)) 직원수						-- 300024
    from salaries;
-- 문제5
-- 부서는 총 몇 개가 있나요?
	select count(*) 부서수
...
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
	select count(distinct(emp_no)) '현재 매니저 수'
    from dept_manager
    where to_date >= now();
-- 문제7
